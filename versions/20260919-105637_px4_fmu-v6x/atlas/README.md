...
The first request requires disarmed, landed, fresh finite attitude/rates and
...
mode switch still releases control. GPS position is not read by this path.

There is no altitude target, position hold, automatic contact detection, or
...
Validation so far: standalone state-gate tests and SITL compilation. The
