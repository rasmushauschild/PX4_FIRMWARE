...
#define NL_ERR(...) do { PX4_ERR(__VA_ARGS__); mavlink_log_critical(&_mavlink_log_pub, __VA_ARGS__); } while (0)   // Aborting phase entered only to keep lockstep alive after an external disarm
 ...
 void Run() override {
  ...
  pilot.active=_pilot_mode && _phase>=Prime && _phase<=LowerNose;
  ...
  if (take(_request)) {
   ...
   _pilot_mode=_rc_throttle.get()!=0; _pilot_gate.reset(); _pilot_cap=0.f; _ready_announced=false; _flight_alt=_pilot_mode ? 1.f : _alt.get();
  ...
  if (_phase==Lift || _phase==Spool) {
   ...
   if (_phase==Lift && elapsed>(_pilot_mode ? 25.f : _timeout.get())) {
    if (_pilot_mode) { pilot_abort(att,pos,now); return; }
