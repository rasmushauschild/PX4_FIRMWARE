// Included inside AtlasNoseLift. This prototype is unreachable in hardware builds.
 bool _manual_selected{false}, _manual_latched{false};
 AtlasManualGate _manual{};
 hrt_abstime _manual_time{}, _manual_start{}, _manual_last_command{};
 ...
 void run_manual() {
  ...
  if(_manual.state==AtlasManualGate::Nose) {
   ...
   if(_manual.confirm(request,stable,now)) {
    _manual_yaw=e.psi(); _manual_pitch=e.theta(); _manual_start=now;
  ...
  if(_manual.state==AtlasManualGate::Flight) {
   ...
   const float thrust=_manual.slew(throttle,dt);
   _manual_yaw=matrix::wrap_pi(e.psi()+sticks[3]*math::radians(30.f)*dt);
   ...
   // Keep existing front support until attitude allocation takes over, but never above RC permission.
   atlas_nose_lift_floor_s feedback{}; _motors_sub.copy(&feedback);
   if(feedback.timestamp>_manual_start && now-feedback.timestamp<100_ms && feedback.active &&
      feedback.control[8]>=.95f*_manual_front9 && feedback.control[9]>=.95f*_manual_front10) {
    _manual_front9=math::max(0.f,_manual_front9-dt*.2f); _manual_front10=math::max(0.f,_manual_front10-dt*.2f);
   }
   ...
   floor.control[8]=math::min(_manual_front9,throttle); floor.control[9]=math::min(_manual_front10,throttle); _floor_pub.publish(floor);
  } else {
